...
public class ProductAliasValueProvider implements FieldValueProvider, Serializable {
    ...
    protected List<FieldValue> createFieldValue(final ProductModel product, final LanguageModel language, final IndexedProperty indexedProperty)
    {
        ...
        final String aliasName = getAliasName(product, language);
        if (aliasName != null)
        {
            // add Unit name value to the fieldValues list
            addFieldValues(fieldValues, indexedProperty, language, aliasName);
        }
        return fieldValues;
    }
    ...
    private String getAliasName(ProductModel product, LanguageModel language) {
